        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {subtareas.map((subtarea) => (
              <div
                key={subtarea.id}
                className="flex items-center gap-2 p-2 border rounded hover:bg-accent/50 transition-colors group"
              >
                <GripVertical className="w-4 h-4 text-muted-foreground cursor-move" />
                
                <Checkbox
                  checked={subtarea.completada}
                  onCheckedChange={() => handleToggle(subtarea.id, subtarea.completada)}
                />
                
                <span className={`flex-1 ${subtarea.completada ? 'line-through text-muted-foreground' : ''}`}>
                  {subtarea.titulo}
                </span>

                <Select
                  value={subtarea.asignado_a || 'unassigned'}
                  onValueChange={(value) => handleAssign(subtarea.id, value === 'unassigned' ? '' : value)}
                >
